...
import {CookieService, isNil} from 'ngx-cookie';
...
import {defaultGetRecordItemsParams, GetRecordItemsParams} from './params/get-record-items-params';
...
export class ApiService extends RestClient {
    ...
    public checkAuthLoginToken(slug: string, token: string): Promise<string> {
        const result = this.addQueryParam('Slug', slug)
    ...
    public resetPassword(token: string, slug: string, password: string): Promise<any> {
        ...
        const result = this.post<TokenResult>(data, true, 'auth/resetpassword/' + '?apitoken=' + token, false).catch(
            (error) => {
                return '';
            }
        );
    ...
    public validatePassword(email: string, oldPassword: string, password: string): Promise<string> {
        const result = this.addQueryParam(
            'Fields',
    ...
    @Path('custom/CustomCalendar/getCalendarMembers/')
    public getCalendarMembers(): Promise<Member[]> {
    ...
    public getRecordVersions(recordSlug: string, version): Promise<any> {
        return this.setUrlParam({recordSlug})
    ...
    public showRecordVersion(recordSlug: string, version: string = null, translateLabels: string = null): Promise<any> {
        return this.setUrlParam({recordSlug})
            .addQueryParam('translateLabels', translateLabels)
            .addQueryParam('Version', version)
            .get();
    }
    ...
    public getRecordFormData(
        slug: string,
        itemSlug: string = null,
        recordVersion: string = null,
        recordItemVersion: string = null,
        translateLabels: string = null
    ): Promise<CustomRecordFormData> {
        ...
        return this.get<CustomRecordFormData>().then((formData) => {
  ...
  */
    ...
    public printRecordItem(itemSlug: string, templateSlug: string) {
        return this.setUrlParam({itemSlug})
            .addQueryParam('templateSlug', templateSlug)
            .get<string>(true);
    }

    @Path('custom/CustomRecordSet/getRecordItems/:recordSlug')
    public getRecordItems(getRecordItemsParams: GetRecordItemsParams): Promise<RecordItems> {
        ...
        this.setUrlParam({recordSlug: params.recordSlug});
        this.addQueryParam('Page', params.page);
        this.addQueryParam('Length', params.count);
        this.addQueryParam('OrderColumn', params.sortColumn);
        this.addQueryParam('OrderOpt', this.getOrderOptFromSortOrder(params.sortOrder));
        const filter = new FilterParam();

        if (!isNil(params.searchTerm) && params.searchTerm.length > 0) {
            filter.addValue('ItemData.Value:PartialMatch', params.searchTerm);
        }

        if (params.recordItemSlugs.length > 0) {
            filter.addValues('Slug', params.recordItemSlugs);
        }

        if (!isNil(params.dateFrom)) {
            filter.addValue('Created:GreaterThanOrEqual', params.dateFrom);
        }

        if (!isNil(params.dateTo)) {
            filter.addValue('Created:LessThanOrEqual', params.dateTo);
        }

        if (!isNil(params.calendarMember)) {
            filter.addValue('ItemData.Value:PartialMatch', params.calendarMember.Slug);
        }
        ...
        if (!isNil(params.filters)) {
            params.filters.forEach((value, key) => {
                ...
                filter.addValue('ItemData.Value:PartialMatch', value);
                filter.addValue('ItemData.FormField.Slug', key);
            });
        }

        this.addFilterParam(filter);
        return this.get<RecordItems>().then((recordItems) => {
            recordItems.Record = this.convertRecordFormDataPlainToClass(recordItems.Record);
            return recordItems;
        });
    }
    ...
    public getRecordItemsFromList(
        ...
        const filter = new FilterParam().addRecordSlug(recordSlug);
    ...
    public getRecordItemsCount(recordSlug: string): Promise<number> {
        return this.addFilterParam(new FilterParam().addRecordSlug(recordSlug)).get<number>();
    }
    ...
    public getRecordItem(recordItemSlug: string, recordItemVersion: string = null): Promise<RecordItemData> {
        if (recordItemVersion !== null) {
            return this.setUrlParam({recordItemSlug})
                .addQueryParam('Version', recordItemVersion)
                .get<RecordItemData>();
        } else {
    ...
    public deleteRecordItem(recordItemSlug: string, deletionType: string = 'logically') {
        return this.setUrlParam({recordItemSlug})
            .addQueryParam('DeleteType', deletionType)
            .delete();
    }

    @Path('RecordItemData')
    public putRecordItemFieldValue(recordItemSlug: string, formFieldSlug: string, value: any) {
        const filter: FilterParam = new FilterParam().addRecordItemSlug(recordItemSlug).addFormFieldSlug(formFieldSlug);
    ...
    public getFormFieldsForModel(modelName: string): Promise<FormData> {
        return this.setUrlParam({modelName})
            .get<FormData>()
            .then((formData) => {
    ...
    public getFormFieldsForModelObject(modelName: string, itemSlug?: string): Promise<FormData> {
        ...
        return this.get<FormData>().then((formData) => {
            formData = plainToClass(FormData, formData);
            formData.Sections = this.convertFormDataSectionsPlainToClass(formData.Sections);
            return formData;
        });
  ...
  public getPrevAndNextModelItem(modelName: string, itemSlug: string): Promise<any> {
    ...
    public deleteFile(fileSlug: string) {
        return this.setUrlParam({fileSlug})
            .addQueryParam('DeleteType', 'physically')
            .delete();
    }
    ...
    public findByIdEveryWorkflowDefinition(
        ...
        this.addQueryParam('Length', count);
        if (searchTerm !== null) {
            this.addQueryParam('searchTerm', searchTerm);
        }
        if (searchTerm !== null) {
            this.addQueryParam('createdFrom', createdFrom);
        }
        if (searchTerm !== null) {
            this.addQueryParam('createdTo', createdTo);
        }
    ...
    public postAddNoteToFile(fileSlug: string, noteContent: string) {
        return this.addFormDataParam('fileSlug', fileSlug)
    ...
    public getFolderTree(folderSlug: string, itemSlug: string) {
        return this.addFormDataParam('currentFolder', folderSlug)
    ...
    public getFolderFiles(
        ...
        return this.addFormDataParam('currentFolderSlug', currentFolderSlug)
            .addFormDataParam('itemSlug', itemSlug)
    ...
    public postSetFolder(
        ...
        this.addFormDataParam('folderName', folderName)
