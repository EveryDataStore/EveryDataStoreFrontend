...
export class RelationFieldComponent extends BaseFieldComponent implements OnInit {
    ...
    private async fillFieldValuesFromMapping() {
        ...
        await this.pluginService.fire(
            Hooks.AfterFilledRelationFieldValuesFromMapping,
            new PluginServiceData(this, itemValues)
        );
    }
    ...
    private async createItemsFromRecord(recordSlug: string) {
        ...
        this.recordItems = await this.apiService.getRecordItems({
            recordSlug,
            page: 1,
            count: itemsCount,
        });
